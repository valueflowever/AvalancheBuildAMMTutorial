...
describe('application reducer', () => {
  ...
  describe('updateBlockNumber', () => {
    it('updates block number', () => {
      store.dispatch(updateBlockNumber({ chainId: ChainId.tAVAX, blockNumber: 4 }))
      expect(store.getState().blockNumber[ChainId.tAVAX]).toEqual(4)
    })
    it('no op if late', () => {
      store.dispatch(updateBlockNumber({ chainId: ChainId.tAVAX, blockNumber: 2 }))
      expect(store.getState().blockNumber[ChainId.tAVAX]).toEqual(3)
    })
