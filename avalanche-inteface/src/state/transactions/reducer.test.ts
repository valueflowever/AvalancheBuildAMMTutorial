...
describe('transaction reducer', () => {
  ...
  describe('addTransaction', () => {
    it('adds the transaction', () => {
      ...
      expect(txs[ChainId.tAVAX]).toBeTruthy()
      expect(txs[ChainId.tAVAX]?.['0x0']).toBeTruthy()
      const tx = txs[ChainId.tAVAX]?.['0x0']
  ...
  describe('clearAllTransactions', () => {
    it('removes all transactions for the chain', () => {
      ...
      expect(Object.keys(store.getState())).toEqual([String(ChainId.tAVAX), String(ChainId.RINKEBY)])
      expect(Object.keys(store.getState()[ChainId.tAVAX] ?? {})).toEqual(['0x0'])
      expect(Object.keys(store.getState()[ChainId.RINKEBY] ?? {})).toEqual(['0x1'])
      store.dispatch(clearAllTransactions({ chainId: ChainId.tAVAX }))
      expect(Object.keys(store.getState())).toHaveLength(2)
      expect(Object.keys(store.getState())).toEqual([String(ChainId.tAVAX), String(ChainId.RINKEBY)])
      expect(Object.keys(store.getState()[ChainId.tAVAX] ?? {})).toEqual([])
      expect(Object.keys(store.getState()[ChainId.RINKEBY] ?? {})).toEqual(['0x1'])
    })
  })
})
